...
Shiny extend the ruby core string class with two methods, ansi and html. These two are proxy methods to provide a bunch of color and text formating methods.
Over ansi there are the commen known ansi escape sequences available, bold, blink, underline and the whole colorful format things. The method html serve the same but in the html way, it makes the
...
Helpers to list available colors and effect methods

  Shiny.colors  # returns array with colors
  Shiny.effects # returns array with effects

  # call it in terminal to see it in action
  Shiny.show_colors
  Shiny.show_effects
